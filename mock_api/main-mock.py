...
def execute_restore_command(restore_mode: str) -> bool:
    ...
    if restore_mode == "parallel":
        time.sleep(85*0.001)
    else:
        time.sleep(3135*0.001)
    
    try:
        # 构建命令
        sdir = "para-251103-0942" if restore_mode == "parallel" else "seq-251103-0938"
        commands = [
            f"cp ./../mitosis_log/{sdir}/* /etc/mitosis/"
        ]
...
def invoke(req: InvokeRequest) -> List[Hotel]:
    ...
    # 确保目录存在
    os.makedirs(os.path.dirname(log_file), exist_ok=True)
    os.makedirs("/etc/mitosis/", exist_ok=True)
    ...
    if is_fast_start:
        ...
        time.sleep(85*0.001)
        cmd = "cp ./../mitosis_log/para-251103-0942/* /etc/mitosis/"
    else:
        # 普通启动模式
        write_log_message("Using default start approach...")
        time.sleep(3135*0.001)
        cmd = "cp ./../mitosis_log/seq-251103-0938/* /etc/mitosis/"
